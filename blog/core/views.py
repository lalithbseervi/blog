...
class BlogPostService:
    ...
    def getRelatedPosts(user: User, slug) -> list:
        ...
        if user.is_superuser:
            ...
            for category in categories:
                related_posts += (BlogPost.objects.filter(categories=category).exclude(slug=slug).filter(password_protect=False))

        seen = set()
        unique_related_posts = []
        for post in related_posts:
            if post not in seen:
                unique_related_posts.append(post)
                seen.add(post)

        related_posts = unique_related_posts
